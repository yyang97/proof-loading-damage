...
nlogpost_checkRT <- function(theta,R_group,T_group,
  ...
  eta <- expit(theta[6])
  alpha <- theta[7:8]
  lik <- PFY_lik_R(mu,sigma,rho,eta,alpha[1],R_pl,R_group) +
    PFY_lik_R(c(mu[2],mu[1]),c(sigma[2],sigma[1]),rho,eta,
              alpha[2],T_pl,T_group) +
...
nlogpost_plot <- function(theta){
  nlogpost_full(theta,data)
}
nlogpost_full_fixeta <- function(theta,data,eta){
  mu <- theta[1:2]
  sigma <- theta[3:4]
  rho <- expit(theta[5])
  eta <- expit(eta)
  alpha <- theta[6:8]
  lik <- PFY_lik_R(mu,sigma,rho,eta,alpha[1],R_pf[1],data[[1]]) +
    PFY_lik_R(mu,sigma,rho,1,0,R_pf[2],data[[2]]) +
    PFY_lik_R(mu,sigma,rho,eta,alpha[2],R_pf[3],data[[3]]) +
    PFY_lik_R(c(mu[2],mu[1]),c(sigma[2],sigma[1]),rho,1,0,T_pf[1],data[[4]]) +
    PFY_lik_R(c(mu[2],mu[1]),c(sigma[2],sigma[1]),rho,eta,alpha[3],T_pf[2],data[[5]]) +
    PFY_lik_R(c(mu[2],mu[1]),c(sigma[2],sigma[1]),rho,1,0,T_pf[3],data[[6]]) +
    sum(dnorm(data[[7]],
              mean = mu[1],
              sd = sigma[1], log = T)) + 
    sum(dnorm(data[[8]],
